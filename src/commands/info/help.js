...
exports.execute = async (client, message, args, data) => {
	if (!args[0]) {
		const misc = message.client.commands
			.filter((x) => x.help.category == "Misc")
			.map((x) => "`" + x.help.name + "`")
			.join(" ◦  ");
		const music = message.client.commands
			.filter((x) => x.help.category == "Music")
			.map((x) => "`" + x.help.name + "`")
			.join(" ◦  ");
		const eco = message.client.commands
			.filter((x) => x.help.category == "Economy")
			.map((x) => "`" + x.help.name + "`")
			.join(" ◦  ");
		const gw = message.client.commands
			.filter((x) => x.help.category == "Giveaway")
			.map((x) => "`" + x.help.name + "`")
			.join(" ◦  ");
		const mod = message.client.commands
			.filter((x) => x.help.category == "Moderation")
			.map((x) => "`" + x.help.name + "`")
			.join(" ◦  ");
		...
		embed.addField("For games", " Page 8");
		...
		const embedgame = new MessageEmbed()
			...
			.addField(
				"Note",
				"The hosting for the games side of Yarn (seperate to the rest of Yarn) is still under maintenance, so commands may be off for prolonged periods of time.",
			)
			.addField(
				"Commands",
				"`tr` ◦ `chess` ◦ `hangman` ◦ `connect4` ◦ `snake` ◦ `stop`",
			);
