...
// Foreach in objects
const ObjectforEach = function(collection, callback, scope) {
...
exports.execute = async (client, message, args, data) => {
	...
	const key = process.env.HYPIXEL; // CHANGE THIS OR THE BOT WONT WORK! // CHANGE THIS OR THE BOT WONT WORK!
	const prefix = client.prefix; // Change this to yours!
	/* Cooldown */
	const cooldownT = 30 * 1000;
	const cooldownG = cooldowns.hypixel.pit.get(message.author.id);
	if (cooldownG) {
	...
	hypixel.getPlayerByName(key, args[0]).then((user) => {
      ...
      !user.player
		...
		cooldowns.hypixel.pit.set(message.author.id, Date.now() + cooldownT);
		setTimeout(() => cooldowns.hypixel.pit.delete(message.author.id), cooldownT);
		return message.channel.send({ embeds: [embed] });
	});
};
exports.help = {
