...
const leaveCanvas = new canvas.Goodbye();
...
const universalColor = client.config.unicolor.toUpperCase();
const imageLink =
...
const prefix = client.prefix; // Change this to yours!
client.on('guildMemberAdd', async (member) => {
  if (member.guild.id != '658976660703543297') return;
  const image = await welcomeCanvas
...
client.on('messageCreate', async (message) => {
  ...
  if (msgl < 10) {
    ...
    const randomnum = Math.floor(Math.random() * 2 * 100) / 100;
  ...
  // if not too short do this
    ...
    const randomnum2 = 1 + Math.floor(msgl * 100) / 100;
    // basically databasing again
    client.points.math(rankkey, `+`, randomnum2, `points`);
    client.points.inc(rankkey, `points`);
  }
  // get current level
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}rank`)) {
    ...
    // math
    const curpoints = Number(client.points.get(rankkey, `points`).toFixed(2));
    // math
    const curnextlevel = Number(
  ...
  const set = client.db.get(`g_${message.guild.id}`);
