...
exports.execute = async (client, message, args) => {
  ...
  const key = process.env.HYPIXEL; // CHANGE THIS OR THE BOT WONT WORK!
  const token = process.env.TOKEN; // CHANGE THIS OR THE BOT WONT WORK!
  const prefix = client.prefix; // Change this to yours!
  if (message.author.bot) return;
  /* Cooldown */
  const cooldownT = 30 * 1000;
  const cooldownG = cooldowns.hypixel.main.get(message.author.id);
  if (cooldownG) {
        ...
        hypixel
            ...
            !user.player
              ...
              user.player.mostRecentGameType != undefined
                ...
                const lastLogin = new Date(user.player.lastLogin);
                const firstLogin = new Date(user.player.firstLogin);
