...
exports.execute = async (client, message, args) => {
    ...
    if (args[0].toLowerCase() == 'nikes') {
        ...
        let nikeses = await client.db.get(`nikes_${message.guild.id}_${user.id}`)
        ...
        let cars = await client.db.get(`car_${message.guild.id}_${user.id}`)
        ...
        let houses = await client.db.get(`house_${message.guild.id}_${user.id}`)
